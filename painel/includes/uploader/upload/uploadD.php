...
if($upload){
	...
	foreach($temp as $img)
	{
		...
		$sql = "CREATE TABLE IF NOT EXISTS `produtosDocumentos` (`id` int(255) NOT NULL AUTO_INCREMENT, `img` varchar(255), `token` varchar(255), `legenda` varchar(255), `principal` varchar(255), PRIMARY KEY (`id`)) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=0";
		$query = mysqli_query($conn,$sql);
		
		$Sql = "INSERT INTO produtosDocumentos (img, token, legenda) VALUES ('".$nome.".".$ext."', '".$token."', '".$legenda."')";
		$Query = mysqli_query($conn,$Sql);
